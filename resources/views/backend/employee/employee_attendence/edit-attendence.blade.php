            ...
            <div class="card">
              ...
              <div class="card-body">
                    ...
                    <div class="form-group col-md-4">
                            ...
                            <input type="text" id="date" name="date" class="form-control form-control-sm singledatepicker" value="{{date('m-d-Y',strtotime($data[0]->date))}}" placeholder="Enter Your Join Date" autocomplete="off">
                            @error('date')
                        ...
                        <table class="table-sm table-bordered table-striped dt-responsive" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr id="div{{$row->id}}" class="text-center">
                                    <input type="hidden" name="employee_id[]" value="{{$row->employee_id}}" class="employe_id">
                                    <td>{{$key+1}}</td>
                                    <td>{{$row->employee->name}}</td>
                                    <td colspan="3">
                                        <div class="switch-toogle switch-3 switch-candy">
                                            <input class="present"   name="attend_status{{$key}}" id="present{{$key}}" value="present" type="radio" {{($row->attend_status == 'present')?'checked':''}} >
                                            <label for="present{{$key}}">Present</label>
                                            <input class="leave"  type="radio" name="attend_status{{$key}}" id="leave{{$key}}" value="leave" {{($row->attend_status == 'leave')?'checked':''}} >
                                            <label for="present{{$key}}">Leave</label>
                                            <input class="absent" type="radio" name="attend_status{{$key}}" id="absent{{$key}}" value="absent" {{($row->attend_status == 'absent')?'checked':''}} >
